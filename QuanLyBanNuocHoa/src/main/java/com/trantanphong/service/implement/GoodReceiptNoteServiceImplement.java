...
public class GoodReceiptNoteServiceImplement implements GoodReceiptNoteService{
	...
	public GoodReceiptNoteDTO save(GoodReceiptNoteDTO newDTO) {
		// TODO Auto-generated method stub
		GoodReceiptNote newEntity = new GoodReceiptNote();
		if(newDTO.getId() != null)
		{
			GoodReceiptNote oldEntity = goodReceiptNoteRepository.findById(newDTO.getId()).orElse(null);
			newEntity = goodReceiptNoteConvert.toEntity(newDTO, oldEntity);
		}
		else
		{
			newEntity = goodReceiptNoteConvert.toEntity(newDTO);
		}
